...
void DFA::Read(const std::string& filename)
{
...
bool DFA::Verification(std::string word)
{
	...
	while (word.size())
	{
		...
		if (std::find(m_sigma.begin(), m_sigma.end(), word[0]) == m_sigma.end() && word.size())
		{
			ok = false;  // exceptie daca ajunge in 2
			//std::cout << "Blocaj"<<std::endl
			break;
		}
		if (std::find(m_states.begin(), m_states.end(), start) == m_states.end() && word.size())
		{
			ok = false;
			//std::cout << "Blocaj"<<std::endl
			break;
