...
void DFA::Read()
{
	std::ifstream inputFile("inputDFA.txt");
...
void DFA::Verification(std::string word)
{
	bool ok = true;
	...
	if (ok == false)
		std::cout << "Obstruction by DFA" << std::endl;
	else {
		if (std::find(m_finals.begin(), m_finals.end(), start) != m_finals.end() && word.size() == 0)
			std::cout << "Acceptable by DFA" << std::endl;
		else
			std::cout << "Unacceptable by DFA" << std::endl;
	}
...
std::set<char> DFA::GetSigma() const
{
	return m_sigma;
}
...
std::vector<Transition> DFA::GetTransition() const
{
	return m_transition;
}

std::set<std::string> DFA::GetFinals() const
{
	return m_finals;
}
